<?php if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly    
	...
	$_total_posts = $this->getTotalPosts( $_date_format, $category_id, 1, 0 );
	if( $_total_posts <= 0 ) {
	...
	if( ( $all_pg ) >= $cur_all_pg + 2 ) {
		?>
			<div class="clr"></div>
			<div class='ik-post-load-more' align="center" onclick='AVPT_loadMorePosts("<?php echo esc_js( $_date_format ); ?>","<?php echo esc_js( $_limit_start+$_limit_end ); ?>","<?php echo esc_js( $params["vcode"]."-".$_date_format ); ?>","<?php echo esc_js( $_total ); ?>",request_obj_<?php echo esc_js( $params["vcode"] ); ?>)'>
