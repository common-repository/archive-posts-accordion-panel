<?php if ( ! defined( 'ABSPATH' ) ) exit; 
	 $params = $_REQUEST;  
	 $_date_format = ( isset( $params["date_format"] ) ? intval( $params["date_format"] ) : 0 ); 
	...
	$_total_posts = $this->getTotalPosts( $_date_format, $category_id, 1, $is_default_category_with_hidden );
	if( $_total_posts <= 0 ) {
		?><div class="ik-post-no-items"><?php echo __( 'No posts found.', 'archivespostaccordion' ); ?></div><?php
		die();
	} 
	$post_list = $this->getPostList( $category_id, $_date_format, $_limit_end );	 
	...
	foreach ( $post_list as $_post ) { 
		...
		<div class='ik-post-item pid-<?php echo esc_attr( $_post->post_id ); ?>'> 
			<div class='ik-post-image' onmouseout="avpt_pr_item_image_mouseout(this)" onmouseover="avpt_pr_item_image_mousehover(this)">
	...
	if( $_total_posts > sanitize_text_field( $params["number_of_post_display"] ) ) { ?>
			<div class="clr"></div>
			<div class='ik-post-load-more'  align="center" onclick='AVPT_loadMorePosts( "<?php echo esc_js( $_date_format ); ?>", "<?php echo esc_js( $_limit_start+$_limit_end ); ?>", "<?php echo esc_js( $params["vcode"]."-".$_date_format ); ?>", "<?php echo esc_js( $_total_posts ); ?>", request_obj_<?php echo esc_js( $params["vcode"] ); ?> )'>
